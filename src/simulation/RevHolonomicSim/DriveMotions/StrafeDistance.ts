...
import { clamp_min_speed, slew, to_holonomic_voltage, wrap_angle_180 } from "../../RevSim/Util";
...
export function reset_strafe_distance() {
    lateralPID?.reset();
    angularPID?.reset();
    start_x = 0;
    start_y = 0;
...
export function strafe_distance(robot: Robot, dt: number, distance: number, target_heading: number | null, p: revConstants[]): boolean {
    const lat = p[0];
    const ang = p[1];
    ...
    const traveled = (robot.getX() - start_x) * Math.cos(toRad(heading))
        - (robot.getY() - start_y) * Math.sin(toRad(heading));
